...
export default async function HomePage() {
  ...
  const [packagesRes, testimonialsRes, galleryRes, heroSlidesRes] = await Promise.all([
    supabase
      .from('packages')
      .select('id, name, price, locations(location_images(image_url)), activities(name), facilities(name)')
      .eq('is_featured', true)
      .limit(4),
                  ...
                  <div className="border rounded-lg shadow-lg flex flex-col overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 h-full">
                    {pkg.locations?.location_images?.[0]?.image_url && (
                      <div className="relative w-full h-48">
                        <Image
                          src={pkg.locations.location_images[0].image_url}
                          alt={pkg.name}
                          fill
                    ...
                    <div className="p-6 flex flex-col flex-grow">
                      ...
                      <ul className="space-y-2 text-gray-600 mt-4 flex-grow">
                        {[...pkg.activities, ...pkg.facilities].slice(0, 5).map((item, itemIndex) => (
                          <li key={`${item.name}-${itemIndex}`} className="flex items-center gap-2">
                      ...
                      <Link
                        ...
                        className="mt-6 block bg-green-500 text-white text-center font-bold py-2 px-4 rounded-md hover:bg-green-600"
                      >
                        Click Here
                      </Link>
                    </div>
      ...
      <section className="relative w-full h-screen flex items-center justify-center text-center text-white overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-6">
          <h1 className="text-4xl md:text-6xl font-bold animate-fade-in-up">
            {settingsMap.get('hero_title')}
          </h1>
          <p
            className="text-lg md:text-xl mt-4 animate-fade-in-up"
            style={{ animationDelay: '0.5s' }}
          >
            {settingsMap.get('hero_subtitle')}
          </p>
          <Link
            href="/packages"
            className="mt-8 inline-block bg-orange-500 font-bold py-3 px-6 rounded-full hover:bg-orange-600 transition-transform hover:scale-105 animate-fade-in-up"
